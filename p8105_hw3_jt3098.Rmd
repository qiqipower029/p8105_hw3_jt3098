...
* It provides information about record ID, date, precipitation, snowfalls, the minimum temperature and the maximum temperature of day, etc.

```{r observed most in snowfall}
ny_noaa %>%
  filter(!is.na(snow)) %>%
  group_by(snow) %>%
    summarise(n_value = n()) %>%
